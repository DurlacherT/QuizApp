  ...
  , ];
    ...
    async startQuiz() {
        try {
            ...
            var result = [1, 0][Math.floor(Math.random() * 2)];
            let data;
            let type;
            if (result == 1) {
                type = 1
...
class Quiz {
    ...
    nextQuestion() {
        const checkedElement = this.questions[this.answeredAmount].answerElements.filter(el => el
            .firstChild.checked);
    ...
    showResult() {
        this.questions[this.answeredAmount].isCorrect ? alert('Correct answer :)') : alert(
            'Wrong answer :(');
    }
    endQuiz() {
...
class Final {
    ...
    renderScoreboard(name, questions, score) {
        this.userdataElement.appendChild(document.createTextNode(
            `${name} answered ${questions} out of ${score} questions correct!`));
    ...
    async fetchUsers() {
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                this.renderScoreboard(JSON.stringify(data[i].username), JSON.stringify(data[i].score),
                    JSON.stringify(data[i].question));
            }
        } catch (error) {
            alert(error);
        }
